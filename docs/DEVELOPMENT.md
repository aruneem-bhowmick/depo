...
└── components/
    ...
    ├── ConfirmGate.test.tsx     ConfirmGate: label/count rendering, singular/plural button text, correct/wrong input matching, whitespace rejection, animate-shake add/remove, loading state, programmatic-click guard (13 cases)
    ...
    ├── DeleteProgress.test.tsx  DeleteProgress: indeterminate progress bar, count text, results rendering, checkmark/X icons, error message, onComplete callback, fetch error alert, non-ok HTTP response alert, onComplete not called on non-ok response, POST body (10 cases)
