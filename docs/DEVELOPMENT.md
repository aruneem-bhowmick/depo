...
└── components/
    ...
    ├── ConfirmPage.test.tsx     /confirm page: empty/invalid sessionStorage redirect, repo list rendering, heading count (singular/plural), ConfirmGate→DeleteProgress transition, CommandOutput for gh/curl modes, sessionStorage write and clear on completion (12 cases)
    ├── DeleteProgress.test.tsx  DeleteProgress: indeterminate progress bar, count text, results rendering, checkmark/X icons, error message, onComplete callback, fetch error alert, POST body (8 cases)
